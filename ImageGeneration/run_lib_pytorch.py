...
# 两个东西，sampling_fn 和 train_step_fn 
...
def train(config, workdir):
    ...
    # Initialize model.
    score_model = mutils.create_model(config)  # HERE！！！
    ...
    train_ds_loader = torch.utils.data.DataLoader(train_ds, batch_size=config.training.batch_size, shuffle=True,
                                                  drop_last=True, num_workers=4)
    eval_ds_loader = torch.utils.data.DataLoader(eval_ds, batch_size=config.training.batch_size, shuffle=True,
                                                 drop_last=True, num_workers=4)
...
def evaluate(config,
    ...
    if config.training.sde.lower() == 'rectified_flow':
        ...
        sde = sde_lib.RectifiedFlow(init_type=config.sampling.init_type, noise_scale=config.sampling.init_noise_scale,
                                    use_ode_sampler=config.sampling.use_ode_sampler,
                                    sigma_var=config.sampling.sigma_variance, ode_tol=config.sampling.ode_tol,
    ...
    for ckpt in range(begin_ckpt, config.eval.end_ckpt + 1):
        ...
        if config.eval.enable_figures_only:
            ...
            for r in range(num_sampling_rounds):
                ...
                # Directory to save samples. Different for each host to avoid writing conflicts
                samples, n = sampling_fn(score_model)
                torchvision.utils.save_image(samples.clamp_(0.0, 1.0), os.path.join(this_sample_dir, '%d.png' % r),
